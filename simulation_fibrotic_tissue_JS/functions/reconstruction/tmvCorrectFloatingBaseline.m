...
madSig = mad(tmv,1,1);
madSig = gaussFiltfilt(madSig,1);
[~,peakLoc] = findpeaks(madSig, 'MinPeakProminence',0.1*max(madSig), 'MinPeakDistance', min(200,size(madSig,2)-2), 'sortstr','descend'); % sinus, pacing
% [~,peakLoc] = findpeaks(madSig, 'MinPeakHeight',0.1*max(madSig), 'MinPeakDistance', min(50,size(madSig,2)-2), 'sortstr','descend'); % VT
