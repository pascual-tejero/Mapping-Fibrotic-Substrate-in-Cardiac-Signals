function [L,inds,b,c]=TransferMatrix_Phi_Constant(meshes,ci,co,zerolevel,b_in,omegas)
% function [L,inds,b,c]=TransferMatrix_Phi_Constant(meshes,ci,co,zerolevel, b_in, omegas)
% Builds constant transfer matrix for Phi in case of many boundary surfaces.
...
% all triangle centroids", or I for "sum of all triangle centroids on surface I".
...
%
% omegas (optional)
% cell array of double layer matrices as provided by DLMatrices_Constant
%
...
c=zeros(nsurf);
sind(1)=1;%index for the first triangle of surface I
noe=0;%number of triangles in the whole model
for I=1:nsurf,
    noe=noe+meshes{I}.noe;
    eind(I)=sind(I)+meshes{I}.noe-1;%index for the last triangle of surface I
    if I<nsurf
        sind(I+1)=eind(I)+1; %index for the start triangle for surface I+1
    end
    inds{I}=sind(I):eind(I);%indices for triangles on surface I
end
...
for I=1:nsurf,
    ...
    for J=1:nsurf
        ...
        if flag_calcomegas
            if I==J && c(I,J)~=0
                omegas{I,J}=DLMatrix_Constant_SameSurface(meshes{I},1);%Eq. 25
            elseif I~=J && c(I,J)~=0 
                omegas{I,J}=DLMatrix_Constant_DiffSurface(meshes{I},meshes{J},1);%Eq. 25
            else
                omegas{I,J}=0;
            end
        end
...
if nargin>=4 && ~isempty(zerolevel) && zerolevel>-1
    ...
    if zerolevel==0
        ...
        w(sind(zerolevel):eind(zerolevel))=1/meshes{zerolevel}.noe;
    end
    defmatrix=ones(noe,1)*w;
    Linv=Linv+defmatrix;
    clear defmatrix
end
L=inv(Linv);
