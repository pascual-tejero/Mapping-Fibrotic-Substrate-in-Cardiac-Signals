...
def run(args, job):
    ...
    geom = mesh.Block(size=(50, 50, 1), resolution=0.6) #It is created an absolute path where it is stored a file of the mesh
    ...
    points_text[:,2] = points_text[:,2] + 1500 #It is added the position of the 3rd column of the file to 550 µm 
    print(points_text)
    ...
    #Ionic setup
    imp = ['-num_imp_regions',1, '-imp_region[0].im','COURTEMANCHE','-imp_region[0].num_IDs',1, '-imp_region[0].ID',0]
    ...
    cv=['-num_gregions',1,'-gregion[0].num_IDs',1,'-gregion[0].ID',0,'-gregion[0].g_il',0.1438,'-gregion[0].g_it',0.1438,'-gregion[0].g_in',0.1438]
