...
phie = igb_data('/Volumes/Daten/Benutzer/pt732/Experiments/03_control_reentry/Simulations_r02_6/d1500_e16x16/2021-08-19_basic_3000.0/phie_recovery.igb', 'all', 'all');
...
% lambda = lambda/10;
[Xm,~,~] = tikhonov_gsvd(B,U,Q,S,M,lambda);
...
[AT_reconstruction,RT_reconstruction] = calculation_AT_RT(Xm,cut_simulation,10);
...
%%
filename = 'case_healthytissue_reentry_d1_16x16.xlsx';
...
% points_TR_points = TR_points.Points(TR_points.Points(:,3) == distance_tissue,:);[singularity_points] = singularity_points(values, mesh)
% 
...
qtrip(comb.points, comb.cells, Xm, 1041, [0 90 90], 0.1, [min(min(Xm)) max(max(Xm))], 9);
pause(0.5)
...
qtrip(comb.points, comb.cells, singularity_points, 1041, [0 90 90], 0.5, [min(min(singularity_points)) max(max(singularity_points))], 10);